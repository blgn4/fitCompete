...
def generate_user_profiles():
	...
	for key, value in user_date.iteritems():
		que1 = "select mean(speed),mean(calories_rate),mean(heart_rate) from fitbit_data where user_id='"+str(key)+"' and date='"+value 
		que2="select user_id,bmi,fat,steps,floors,calories,total_time,date from fitbit_data where user_id='"+str(key)+"' and date='"+value

		res1=client.query(que1)
